from flask_restx import Resource, Namespace #Namespace acts like a blueprint in Flask
from .extensions import db
from .api_models import course_model, student_model, course_input_model, student_input_model
from .models import Course

ns = Namespace("api") #could put URI 

@ns.route("/hello") #defines an endpoint "/hello" class; a class as a decorator; inherits from "Resource" that was imported
def get(self):
    return {"hello": "rest-x"}
...
class CourseListAPI(Resource):
  ...
  def get(self): #defines a method for a HTTP method
    return Course.query.all() #returns a JSON seralizable object (dictionary)

@ns.expect(course_input_model)
@ns.marshal_with(course_models)
def post(self):
  print(ns.payload)
